...
// Filtre yachts réutilisable pour pages sous-région (généralisé depuis YachtFiltersCaribbean)
// Le dropdown destination peut être désactivé via showDestination={false}
...
export default function YachtFiltersRegion({
  filters,
  onChange,
  destinations = [],
  ...
  const handleReset = () => {
    const empty = {
      type: '', destination: defaultDestinationValue, capacity: '',
  ...
  const activeCount = Object.entries(localFilters).filter(([key, v]) => {
    if (key === 'destination' && v === defaultDestinationValue) return false;
    if (key === 'currency' && v === '') return false;
    return v && v !== '' && v !== false;
  }).length;

  const showDestinationDropdown = destinations.length > 0;
