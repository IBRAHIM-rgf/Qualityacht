...
function JetModal({ dest, onClose }) {
  ...
  const isCaribbean = dest.name === 'Caribbean';
  ...
  return (
    ...
    >
      <style>{`
        ...
        .modal-scroll::-webkit-scrollbar-thumb { background: #c0c0c0; border-radius: 2px; }
        .modal-scroll { scrollbar-width: thin; scrollbar-color: #c0c0c0 transparent; }

        .cloud-modal {
          background-image: url('/images/nuagesAncien.png');
      ...
      <div
        className={`modal-scroll relative w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-2xl border border-white/10 px-6 py-8 md:px-10 md:py-10 ${isCaribbean ? 'cloud-modal' : ''}`}
        style={!isCaribbean ? { backgroundColor: '#0a1432' } : {}}
        onClick={e => e.stopPropagation()}
      >
        {/* Bouton fermer */}
        <button
          onClick={onClose}
          className="absolute top-4 right-5 text-2xl transition-colors duration-200 cursor-pointer z-10"
          style={{ color: '#c0c0c0', opacity: 0.6 }}
        ...
        {/* Titre du modal */}
        <h2
          className="trajan-regular text-xl md:text-2xl uppercase tracking-[0.1em] mb-6 text-center"
          style={{ color: '#c0c0c0' }}
        >
          {dest.name}
        </h2>
        ...
        <div className="space-y-6">
          {dest.groups.map((group, i) => (
            <div key={i}>
              {/* Nom de l'île/ville — orange brûlé + souligné */}
              <p
                className={`text-sm md:text-base font-bold uppercase tracking-[0.2em] mb-2 text-center island-underline`}
                style={{ color: '#c2622a' }}
              >
                {group.island}
              </p>

              {/* Liste des aéroports — gris argenté */}
              <ul className="space-y-1.5">
                {group.airports.map((airport, j) => (
                  ...
                  >
                    <span
                      className="mt-1 w-1.5 h-1.5 rotate-45 shrink-0 inline-block"
...
// ── Page ───────────────────────────────────────────────────────────────────────
export default function PrivatJetPageV3() {
