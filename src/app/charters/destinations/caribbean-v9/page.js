  ...
  return (
    <div className="border-b border-white/10">
      <button onClick={() => setOpen(o => !o)}
        ...
        <span className={`text-[#c2622a] transition-transform duration-300 text-2xl leading-none ml-3 ${open ? 'rotate-180' : ''}`}>▾</span>
      </button>
      {open && (
  ...
  return (
    <div className="border-b border-white/10">
      <button onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between py-4 text-left group">
        <div className="text-center flex-1">
          <span className="trajan-regular text-[#acb0cd] text-xs md:text-sm uppercase tracking-[0.15em] group-hover:text-[#c2622a] transition-colors duration-300 leading-snug">
            {q}
          </span>
          <div className="w-6 h-[2px] bg-[#c2622a] mt-2 mx-auto" />
        </div>
      ...
      {open && (
        <div className="pb-5 px-1">
          <p className="text-[#acb0cd]/70 text-sm leading-relaxed text-center">{a}</p>
        </div>
      )}
    </div>
  );
...
// ── Page ───────────────────────────────────────────────────────────────────────
export default function CaribbeanV9Page() {
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-x-12 gap-y-6">
              {islandGroups.slice(-2).map((group) => (
                <div key={group.id} className="w-full md:w-[calc((100%-3rem)/3)]">
                  <IslandGroup group={group} defaultOpen={false} />
                </div>
              ))}
            </div>
          </div>
