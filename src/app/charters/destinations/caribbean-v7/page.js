...
// ── Carte rectangulaire (Caribbean Islands) ────────────────────────────────────
...
function CircleCard({ name, image, href }) {
  ...
  return (
    <a href={href} onClick={handleClick} onMouseEnter={activate} onMouseLeave={deactivate}
      onTouchStart={activate} onTouchEnd={deactivate}
      className="flex flex-col items-center shrink-0 snap-center cursor-pointer"
      style={{ width: '140px' }}>
      <div className={`relative w-[120px] h-[120px] md:w-[140px] md:h-[140px] rounded-full overflow-hidden border-2 transition-all duration-300 mb-3 ${lit ? 'border-[#c2622a]/70 scale-105' : 'border-white/20'}`}>
        <Image src={image} alt={name} fill
          className={`object-cover transition-all duration-500 ${lit ? 'brightness-100 grayscale-0 scale-110' : 'brightness-75 grayscale'}`} />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/60" />
      </div>
      <h3 className={`text-xs md:text-sm font-bold text-center uppercase tracking-wide transition-colors duration-300 ${lit ? 'text-[#c2622a]' : 'text-[#acb0cd]'}`}>
        {name}
      </h3>
    </a>
  );
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="absolute inset-0 z-0">
        <Image src="/images/nuagesAncien.png" alt="" fill className="object-cover opacity-10" />
      </div>
...
function RevealBlock({ label, title, sub }) {
  ...
  return (
    <div ref={ref} className="text-center mb-10 md:mb-14 reveal-up">
      ...
      <BurntLine />
      {sub && <p className="text-[#acb0cd]/50 text-sm md:text-base uppercase tracking-[0.1em] md:tracking-[0.15em] px-4">{sub}</p>}
    </div>
  );
}

export default function CaribbeanV7Page() {
    ...
    <>
      ...
      <div className="bg-[#26272a] text-[#acb0cd] overflow-x-hidden">
        ...
        <div className="relative h-screen">
          <Image src="/images/yachts/yatch2.jpeg" alt="" fill priority className="object-cover object-bottom" />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(38,39,42,0.85) 0%, rgba(38,39,42,0) 40%, rgba(38,39,42,0.85) 100%)' }} />
        ...
        {/* CARIBBEAN ISLANDS — rectangles, fond nuages */}
        ...
        {/* POPULAR DESTINATIONS — cercles slider horizontal, fond nuages */}
        <CloudSection className="bg-[#26272a] py-12 md:py-20 px-4 md:px-16">
          <div className="max-w-7xl mx-auto">
            <RevealBlock
        ...
        <div className="relative h-[55vh] md:h-[75vh] overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 md:px-6">
            ...
            <a
              href="/charters"
              className="trajan-regular text-xs md:text-sm uppercase tracking-[0.2em] md:tracking-[0.3em] px-7 md:px-10 py-3 md:py-4 border border-[#c2622a] text-[#c2622a] hover:bg-[#c2622a] hover:text-white transition-all duration-300"
            >
              Explore Yachts
            </a>
          </div>
